...
public class SubGuiStructureOverview extends SubGui {
	...
	public void createControls() {
		...
		if (structure == null) {
			...
			controls.add(new GuiLabel(ChatFormatting.WHITE + "status: " + ChatFormatting.DARK_GREEN + structure.structureID + ChatFormatting.WHITE + ", " + structure.countTiles() + " tile(s)", 0, 3));
			
			if (structure.parent == null)
				controls.add(new GuiLabel(ChatFormatting.WHITE + "no parent", 0, 20));
			else {
				controls.add(new GuiLabel(ChatFormatting.WHITE + "parent: " + (structure.parent.getStructureWithoutLoading() == null ? ChatFormatting.YELLOW + "pending" : ChatFormatting.DARK_GREEN + structure.parent.getStructureWithoutLoading().structureID + ChatFormatting.WHITE + ", " + structure.parent.getStructureWithoutLoading().countTiles() + " tile(s)"), 0, 20));
			...
			for (IStructureChildConnector child : structure.children.values()) {
				scrollBox.addControl(new GuiLabel("id: " + child.getChildID() + ", " + (child.getStructureWithoutLoading() == null ? ChatFormatting.YELLOW + "pending" : ChatFormatting.DARK_GREEN + child.getStructureWithoutLoading().structureID + ChatFormatting.WHITE + ", " + child.getStructureWithoutLoading().countTiles() + " tile(s)"), 0, 5 + 20 * i));
