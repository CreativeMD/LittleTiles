...
public abstract class LittleStructure {
	...
	public boolean loadTiles() {
		if (mainTile != null) {
			...
			if (!tiles.contains(mainTile))
				addTile(mainTile);
			/*
			 * int i = 0; while (i < tilesToLoad.size()) {
	 ...
	 * world.getChunkFromBlockCoords(coord);
	 * if(WorldUtils.checkIfChunkExists(chunk)) { //chunk.isChunkLoaded TileEntity
	 * tileEntity = world.getTileEntity(coord); if(tileEntity instanceof
	 * TileEntityLittleTiles) { LittleTile tile = ((TileEntityLittleTiles)
	 * tileEntity).getTile(pos.position); if(tile != null && tile.isStructureBlock)
	 ...
	 * pos.coord.posZ); if(!(chunk instanceof EmptyChunk)) { //chunk.isChunkLoaded
	 * TileEntity tileEntity = world.getTileEntity(pos.coord.posX, pos.coord.posY,
	 * pos.coord.posZ); if(tileEntity instanceof TileEntityLittleTiles) { LittleTile
	 * tile = ((TileEntityLittleTiles) tileEntity).getTile(pos.position); if(tile !=
