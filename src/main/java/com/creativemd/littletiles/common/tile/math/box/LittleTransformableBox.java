...
public class LittleTransformableBox extends LittleBox {
	...
	public TransformableAxisBox getBox(LittleGridContext context) {
		return new TransformableAxisBox(this, context, context.toVanillaGrid(minX), context.toVanillaGrid(minY), context.toVanillaGrid(minZ), context.toVanillaGrid(maxX), context.toVanillaGrid(maxY), context.toVanillaGrid(maxZ));
	}
				...
				if (faceCache.tiltedStrip1 == null && faceCache.tiltedStrip2 == null) {
					...
					if (faceCache.tiltedStrip1 != null && faceCache.tiltedStrip2 != null) {
						...
						if (cutPlane1 != null && cutPlane2 != null)
							if (BooleanUtils.isFalse(face.isFacingOutwards(true, inverted, tiltedPlanes[j * 2].normal)) && BooleanUtils.isFalse(face.isFacingOutwards(false, inverted, tiltedPlanes[j * 2 + 1].normal)))
