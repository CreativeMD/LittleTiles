		...
		if(event.getOverlayType() == OverlayType.WATER)
		{
            ...
            if(te instanceof TileEntityLittleTiles)
            {
            	...
            	for (LittleTile tile : ((TileEntityLittleTiles) te).getTiles()) {
    				if(tile instanceof LittleTileBlockColored && tile.isMaterial(Material.WATER) && tile.box.getBox(tile.getContext(), blockpos).intersectsWith(bb))
    				{
			...
			if(stack.getItem() instanceof ISpecialBlockSelector)
			{
				...
				if(selector.hasCustomBox(world, stack, player, state, event.getTarget(), result))
				{
		            ...
		            for (int i = 0; i < boxes.size(); i++) {
		            	RenderGlobal.drawSelectionBoundingBox(boxes.get(i).getBox(boxes.context).offset(boxes.pos).expandXyz(0.0020000000949949026D).offset(-d0, -d1, -d2), 0.0F, 0.0F, 0.0F, 0.4F);
					}
