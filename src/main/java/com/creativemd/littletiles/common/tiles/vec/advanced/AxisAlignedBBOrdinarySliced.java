...
public class AxisAlignedBBOrdinarySliced extends CreativeAxisAlignedBB {
	...
	public boolean equals(Object object) {
		if (object.getClass() == AxisAlignedBBOrdinarySliced.class) {
			return ((AxisAlignedBBOrdinarySliced) object).minX == this.minX && ((AxisAlignedBBOrdinarySliced) object).minY == this.minY && ((AxisAlignedBBOrdinarySliced) object).minZ == this.minZ && ((AxisAlignedBBOrdinarySliced) object).maxX == this.maxX && ((AxisAlignedBBOrdinarySliced) object).maxY == this.maxY && ((AxisAlignedBBOrdinarySliced) object).maxZ == this.maxZ && ((AxisAlignedBBOrdinarySliced) object).slice == this.slice;
		}
		return false;
	}
	...
	public boolean intersectsTwoSides(Axis toIgnore, AxisAlignedBB other) {
		if (slice.axis != toIgnore) {
			...
			return (getMax(other, axisOne) > getMin(axisOne) && getMin(other, axisOne) < getMax(axisOne)) && (getMax(other, axisTwo) > getMin(axisTwo) && getMin(other, axisTwo) < getMax(axisTwo));
		}
	...
	public double calculateAxisOffset(AxisAlignedBB other, Axis axis, double offset) {
		if (intersectsTwoSides(axis, other)) {
			...
			// For anyone who reads this, this was a story of a life time spend on the most
			// complicated problem, while solved with brute force, it shows it elegance
			// ingame. just enjoy ;) - n247s
			BoxCorner filledCorner = slice.getFilledCorner();
	...
	public boolean isVecInsideBoxEdgeTwoSides(Vec3d vec) {
		...
		if (RotationUtils.get(one, vec) >= getMin(one) && RotationUtils.get(one, vec) < getMax(one) && RotationUtils.get(two, vec) >= getMin(two) && RotationUtils.get(two, vec) < getMax(two)) {
			...
			// return LittleUtils.greaterThanAndEquals(sizeOne, difOne) &&
			// LittleUtils.greaterThanAndEquals(sizeTwo, difTwo) &&
			// LittleUtils.smallerThanAndEquals(diff, 1);
		}
		return false;
	}
	...
	private boolean intersectsWithOrdinaryTwoAxis(AxisAlignedBB other) {
		...
		if (!(getMax(other, axisOne) > getMin(axisOne) && getMin(other, axisOne) < getMax(axisOne) && getMax(other, axisTwo) > getMin(axisTwo) && getMin(other, axisTwo) < getMax(axisTwo)))
			return false;
		...
		// minVec = RotationUtils.setValue(minVec,
		// getValueOfFacing(ignoreFace.getOpposite()), slice.axis);
		// maxVec = RotationUtils.setValue(maxVec,
		// getValueOfFacing(ignoreFace.getOpposite()), slice.axis);
