...
public class LittleTileCylinder extends LittleTileBox {
	...
	/**relative to min vec
	 * It is two times larger than ordinary grid to support in between positions. In order to get the right position it has to be divided by 2*/
	...
	public boolean isSolid(EnumFacing facing)
	{
		...
		/*if(facing.getAxisDirection() == AxisDirection.POSITIVE) Wrong!
			return getMax(facing.getAxis()) >= (radius + getCenter(axis)) / 2 + getMin(axis);
