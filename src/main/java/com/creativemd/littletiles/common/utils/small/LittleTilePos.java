...
public class LittleTilePos {
	...
	public LittleTilePos(TileEntity te, BlockPos coord)
	{
		this(te.getPos().getX(), te.getPos().getY(), te.getPos().getZ(), coord);
	}
	
	public LittleTilePos(BlockPos origin, BlockPos coord)
	{
		this(origin.getX(), origin.getY(), origin.getZ(), coord);
	}
	
	public LittleTilePos(int baseX, int baseY, int baseZ, BlockPos coord)
	{
		this(coord.getX() - baseX, coord.getY() - baseY, coord.getZ() - baseZ);
	}
	
	protected LittleTilePos(int relativeX, int relativeY, int relativeZ)
	{
		this.coord = new BlockPos(relativeX, relativeY, relativeZ);
	}
	
	public LittleTilePos(int[] array)
	{
		if(array.length > 3)
		{
	...
	{
		return new LittleTilePos(coord.getX(), coord.getY(), coord.getZ());
	}
