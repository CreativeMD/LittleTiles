...
public class SubGuiStructureOverview extends SubGui {
	...
	public void createControls() {
		...
		if (structure == null) {
			...
			controls.add(new GuiLabel(ChatFormatting.WHITE + "status: " + ChatFormatting.DARK_GREEN + (structure.name != null ? structure.name : structure.type.id) + ChatFormatting.WHITE + ", " + structure.count() + " tile(s)", 0, 3));
			
			if (structure.parent == null)
				controls.add(new GuiLabel(ChatFormatting.WHITE + "no parent", 0, 20));
			else {
				controls.add(new GuiLabel(ChatFormatting.WHITE + "parent: " + (structure.parent.getStructureWithoutLoading() == null ? ChatFormatting.YELLOW + "pending" : ChatFormatting.DARK_GREEN + (structure.parent.getStructureWithoutLoading().name != null ? structure.parent.getStructureWithoutLoading().name : structure.parent.getStructureWithoutLoading().type.id) + ChatFormatting.WHITE + ", " + structure.parent.getStructureWithoutLoading().count() + " tile(s)"), 0, 20));
			...
			for (IStructureChildConnector child : structure.children) {
				scrollBox.addControl(new GuiLabel("id: " + child.getChildID() + ", " + (child.getStructureWithoutLoading() == null ? ChatFormatting.YELLOW + "pending" : ChatFormatting.DARK_GREEN + (child.getStructureWithoutLoading().name != null ? child.getStructureWithoutLoading().name : child.getStructureWithoutLoading().type.id) + ChatFormatting.WHITE + ", " + child.getStructureWithoutLoading().count() + " tile(s)"), 0, 5 + 20 * i));
