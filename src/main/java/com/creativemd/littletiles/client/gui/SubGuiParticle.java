	...
	static {
		registerParticleSpreadGuiHandler("random", ParticleSpreadRandom.class, new ParticleSpreadGuiHandler() {
			...
			public void createControls(GuiParent parent, ParticleSpread spread) {
				...
				parent.addControl(new GuiTextfield("speedx", "" + (spread instanceof ParticleSpreadRandom ? ((ParticleSpreadRandom) spread).speedX : 0.1F), 40, 20, 40, 12).setFloatOnly());
				
				parent.addControl(new GuiLabel("speedzLabel", translate("gui.particle.spread.speedz"), 90, 22));
				parent.addControl(new GuiTextfield("speedz", "" + (spread instanceof ParticleSpreadRandom ? ((ParticleSpreadRandom) spread).speedZ : 0.1F), 145, 20, 40, 12).setFloatOnly());
			}
		});
		registerParticleSpreadGuiHandler("circular", ParticleSpreadCircular.class, new ParticleSpreadGuiHandler() {
			...
			public void createControls(GuiParent parent, ParticleSpread spread) {
				...
				parent.addControl(new GuiTextfield("radius", "" + (spread instanceof ParticleSpreadCircular ? ((ParticleSpreadCircular) spread).radius : 0.1F), 40, 20, 40, 12).setFloatOnly());
				
				parent.addControl(new GuiLabel("stepsLabel", translate("gui.particle.spread.steps"), 90, 22));
				parent.addControl(new GuiTextfield("steps", "" + (spread instanceof ParticleSpreadCircular ? ((ParticleSpreadCircular) spread).steps : 30), 145, 20, 40, 12).setNumbersOnly());
			}
		});
	}
